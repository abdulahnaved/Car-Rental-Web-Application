...
<body class="select-none bg-[#1b1b1b]">
    <div class="black">
        <nav class="flex px-6 relative top-0 left-0 bg-[#2c2c2c] w-full p-4 items-center justify-between">
            ...
            <div class="flex gap-7 items-center">
                ...
                <?php else: ?>
                    <a href="Login.php" class="text-white font-medium cursor-pointer">Login</a>
    ...
    <form method="GET" action="" class="flex gap-3 flex-wrap items-center">
        ...
        <!-- Reset button -->
        <button type="button" onclick="window.location.href='profile.php';" class="bg-[#f5c747] rounded-2xl p-2 px-10 hover:opacity-75 font-bold text-lg hover:text-white">
            Reset
        </button>
    </form>
</div>
    ...
    <?php foreach ($filteredCars as $car): ?>
        <div class="flex flex-col h-64 w-72 group relative overflow-hidden bg-cover bg-no-repeat bg-top rounded-xl">
            <img class="image-box" src="<?= $car['image'] ?>" alt="Car Image">
            ...
            <div class="h-16 w-full bg-[#423f4f] absolute bottom-0 flex justify-between px-4 p-2">
                ...
                <a href="car_details.php?id=<?= $car['id'] ?>" class="bg-[#f1c850] px-4 rounded-xl cursor-pointer font-bold text-xl">Book</a>
            </div>
    </div>
    <?php endforeach; ?>
    </div>
